...
class FriendUtilities {
    func calculateFriendStatistics(for friendId: String,
        ...
        let indoorOutdoorRatio = Double(hangouts.filter { $0.isOutdoor }.count) / Double(hangouts.count)
        ...
        let averageBudget = hangouts.map { $0.budget }.reduce(0, +) / Double(hangouts.count)
